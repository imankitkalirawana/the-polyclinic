...
export function OverdueEmail(aid: number, name: string, date: string) {
  return `<body style="margin: 0; padding: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif; background-color: #ffffff; color: #1a1a1a; line-height: 1.5;">
    <div style="max-width: 800px; margin: 0 auto; text-align: center;">
        ...
        <a href="${API_BASE_URL}appointments?aid=${aid}&status=overdue" style="display: inline-block; background-color: #73CD7D; color: white; padding: 16px 32px; text-decoration: none; border-radius: 8px; font-size: 20px; font-weight: 500; transition: background-color 0.3s ease;">
            Reschedule Now 
        </a>
...
export function ConfirmedEmail(
  ...
  return `<body style="margin: 0; padding: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif; background-color: #ffffff; color: #1a1a1a; line-height: 1.5;">
    <div style="max-width: 800px; margin: 0 auto; text-align: center;">
        <h1 style="color: ${status === 'cancelled' ? '#F31260' : '#73CD7D'}; font-size: 42px; margin-bottom: 40px; font-weight: 700;">Your Appointment is <span style="text-transform: capitalize;">${status}</span></h1>
        
        <div style="background-color: ${status === 'cancelled' ? '#F3126010' : '#73CD7D10'}; border-radius: 12px; padding: 16px; margin-bottom: 32px; text-align: left;">
            <p style="font-size: 18px; color: #333; margin-bottom: 16px; line-height: 1.6;">
                <strong style="color: #1d1b48; display: inline-block; width: 180px;">Appointment ID:</strong> 
                #${aid}
            </p>
            ...
            <p style="font-size: 18px; color: #333; margin-bottom: 16px; line-height: 1.6;">
                <strong style="color: #1d1b48; display: inline-block; width: 180px;">Status:</strong> 
                <span style="color: ${status === 'cancelled' ? '#F31260' : '#73CD7D'}; font-weight: 500; text-transform: capitalize;">${status}</span>
            </p>
        </div>
        ...
        <a href="${API_BASE_URL}appointments?aid=${aid}&status=all" style="display: inline-block; background-color: ${status === 'cancelled' ? '#F31260' : '#73CD7D'}; color: white; padding: 16px 32px; text-decoration: none; border-radius: 8px; font-size: 20px; font-weight: 500; transition: background-color 0.3s ease;">
           ${status === 'cancelled' ? 'View Details' : 'Track Now'}
        </a>
