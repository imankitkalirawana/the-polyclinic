      ...
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col gap-2">
            {['Description', 'Test Information', 'Created By', 'Updated By'].map(
              (_section, index) => (
      ...
      <Card>
        <CardBody className="p-0">
          ...
          <div className="grid grid-cols-5 gap-4 border-b bg-default-100 p-4">
            {['Investigation', 'Value', 'Method', 'Unit', 'Inference'].map((header, index) => (
              <Skeleton key={index} className="h-4 w-24" />
            ))}
          </div>
          {/* Table Row */}
          {Array.from({ length: 5 }).map((_, index) => (
            <div key={`table-row-${index}`} className="grid grid-cols-5 gap-4 p-4">
