...
export function SpecificDateManager({ formik }: SpecificDateManagerProps) {
  ...
  const addNewDate = () => {
    ...
    const existingIndex = currentSpecificDates.findIndex(
      (d) => d.date === tomorrow
    );
  ...
  const updateSpecificDateEnabled = (index: number, enabled: boolean) => {
    formik.setFieldValue(
      `availability.specificDates.${index}.enabled`,
      enabled
    );
  };
  ...
  const updateSpecificDateSlot = (
    ...
    const slotIndex = specificDate.slots.findIndex(
      (slot) => slot.id === slotId
    );
  ...
  const addSpecificDateSlot = (dateIndex: number) => {
    ...
    formik.setFieldValue(
      `availability.specificDates.${dateIndex}.slots`,
      updatedSlots
    );
  };
  ...
  const removeSpecificDateSlot = (dateIndex: number, slotId: string) => {
    const specificDate = formik.values.availability.specificDates[dateIndex];
    const updatedSlots = specificDate.slots.filter(
      (slot) => slot.id !== slotId
    );
    const finalSlots =
  ...
  return (
    <div className="space-y-4">
      {formik.values.availability.specificDates.length > 0 && (
        <div>
          {formik.values.availability.specificDates.map(
            (specificDate, dateIndex) => (
              <div key={specificDate.date} className="space-y-3">
                ...
                <div className="grid grid-cols-10 items-center gap-2">
                  {specificDate.enabled ? (
                      ...
                      return (
                        <React.Fragment key={slot.id}>
                          ...
                          <Tooltip
                            delay={1000}
                            content={
                              isLastSlot ? 'Unavailable all day' : 'Unavailable'
                            }
                          >
                            <Button
                              isIconOnly
                          ...
                          {slotIndex === 0 && (
                            <>
                              <Tooltip
                                delay={1000}
                                content="Add another period for this date"
                              >
                                <Button
                                  isIconOnly
                                  type="button"
                    ...
                    <>
                      <div className="col-span-8 text-sm text-default-400">
                        Unavailable
                      </div>
                      <Tooltip
                        delay={1000}
                      ...
                      >
                        <Button
                          ...
                          size="sm"
                          onPress={() =>
                            updateSpecificDateEnabled(dateIndex, true)
                          }
                          radius="full"
                        >
